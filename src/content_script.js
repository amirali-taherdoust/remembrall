...
(async () => {
const black = await send({ type: "is_blacklisted", url: location.href, host: location.hostname });
if (black && black.blacklisted) {
  if (!document.getElementById("__secgate_warning__")) {
    const w = warningToast(location.hostname);
